...
На следующих скриншотах(Рис. 1-10) представленырезультаты работы
программы.

При запуске программы появляется стартовое окно.

![рис. 1](img/interface.png)*Рис.1: Стартовое окно программы*
...
![рис. 5](img/dijbelfloyd.png)*Рис.5: Кратчайшие пути*
...
![рис. 6](img/primakraskalakirpruf.png)*Рис.6: Остовные деревья*
...
![рис. 7](img/network.png)*Рис.7: Сеть* 
...
![рис. 8](img/flow.png)*Рис.8: Нахождение максимального потока и заданного потока минимальной стоимости*
...
![рис. 9](img/fundamental.png)*Рис.9: Фундаментальные циклы*

Метод Шимбелла и достижимость вершин.

![рис. 10](img/shimbell.png)*Рис.10: Метод Шимбелла*
...
c = p/(1-p) и снова генерируем случайное число от 0 до 1.
Повторяем, пока не сгенерируем нужное количество.

![рис. 11](img/binomial1.PNG)*Рис.11: Алгоритм генерации величины* 

![рис. 12](img/binomialgis3.PNG)*Рис.12: Функция вероятности биномиального распределения с параметром 0.4.* 

![рис. 13](img/gis.PNG)*Рис.13: Полученное распределение.*
...
распределение степеней вершин в графе:
        ...
        int x = 0, n = vertexNum - 1;  // вершины у нас будут имть степень от 1 до n
        double p = 0.4, q = 1 - p, c = p / q, r;
        
        Random rand = new Random();
        for (int i = 0; i < vertexNum; i++, x = 0) 
        {  
            p = Math.Pow(q, n);
...
описанному выше. Количество итераций
...
корректирующиеся так: d(x_i) = min(d(x_i), d(x’)+\omega(x',x_i). Также состоит из 2-х этапов.
